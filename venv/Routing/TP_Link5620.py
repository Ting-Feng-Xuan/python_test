...
def find_elenium_by_xpath(browser,path):
    for i in range(20):
        try:
            ...
            time.sleep(2)
    return None
def SetTPLink5620Route(web_path,ssid,pwd,width,channel,ap_mode):
    ...
    find_elenium_by_xpath(browser,"//*[@id=\"lgPwd\"]")
    ...
    for i in range(3):
        ...
        time.sleep(2)
        try:
            click_elenium_by_xpath(browser,"//*[@id=\"wireless2G_rsMenu\"]/label")       #无线设置
    ...
    scol_bar = browser.find_element_by_xpath("/html/body/div[8]/label")
    top_px = 100
    ActionChains(browser).drag_and_drop_by_offset(scol_bar, 0, top_px).perform()
    time.sleep(3)
...
if __name__ == "__main__":
    ...
    width = "20MHz"
    channel = 13
    ap_mode = "11bgn mixed-40MHz/20MHz自动"
    ap_mode = ("11bg mixed-20MHz", "11bgn mixed-40MHz/20MHz自动", "11bgn mixed-20MHz", "11b only-20MHz",
    ...
    for i in range(7):
        print(i)
        SetTPLink5620Route(web_path,ssid,pwd,width,channel,ap_mode[i])